...
- Отмена запросов: TanStack Query передаёт свой `signal` в `fetchOrgTree` — запрос отменяется при размонтировании (отмена мапится в `ApiError` kind `aborted`, таймаут 10с остаётся `network`).
